...
<template>
  <div style="background-color: #f1f2f5; height: 100%; display: flex; position: relative">
    ...
    <div
      class="digital_robots"
      :style="{
        '--a': textareaHeight,
        width: isShowRightNav ? 'calc(100% - 316px)' : 'calc(100% - 88px)',
        'margin-left': isShowRightNav ? '242px' : '72px',
      }"
    >
  ...
  mounted() {
    ...
    if (this.$route.query.eid != undefined) {
      this.$set(
        this.robotMsg,
        "avatar",
        "static/picture/47fd13ce-a451-483f-931b-39b5a1adac7b.png"
      );
      ...
      if (!this.isfinishThreeFive) {
        ...
        if (res != "" && res.content != null) {
          ...
          if (this.chat_type == "10") {
            ...
            this.replyThreeFive = marked(res.content);
          } else {
