<template>
  ...
  >
    ...
    <div class="reg_cont">
      ...
      <div class="reg_right">
            ...
            <div>
              ...
              <div>
                当前算力：<span style="color: blue"
                  >{{ loginInfo.hash_rates.total }}w</span
                >
              </div>
            </div>
          </div>
          ...
          <div class="mem_cont">
            ...
            >
              ...
              >
                <!-- 有效期 -->
                <div class="cont_top">
                  <span class="top_text-valid" v-if="index == 0">性价比首选</span>
                </div>
                <!-- 产品名称 -->
                ...
                <div class="cont-suanli">
                  <div>
                    通用算力{{ handlesuanLi(0, item.universal_hashrate / 10000) }}
                  </div>
                  <div>
                    <div>
                      对话算力{{ handlesuanLi(0, item.directed_hashrate / 10000) }}
                    </div>
                    <div>
                      <el-tooltip
                        effect="dark"
              ...
              <div
                class="cont_btn"
                :style="current == index ? 'background: #1F64FF;color: #fff;' : ''"
              >
        ...
        <div>
          ...
          <div class="mem_cont">
            ...
            >
              ...
              <div
                class="cont_btn"
                :style="current == index + 6 ? 'background: #1F64FF;color: #fff;' : ''"
              >
                {{ handleTime(item.prod_desc.valid) }}
              </div>
            </div>
        ...
        <div class="topPay">
          <div>
            当前选择：<span>{{
              current <= 5 ? datasOne.prod_name : datasTwo.prod_name
            }}</span>
          </div>
          <div>
            <span>¥</span>
            <span>{{ current <= 5 ? datasOne.totalPrice : datasTwo.totalPrice }}</span>
          </div>
          <div>
            <div>
              <span>购买即视为认同</span>
            ...
            <div @click="payVisible = true">付款购买</div>
          </div>
        </div>
        <!-- 支付完成 -->
    ...
    <el-dialog
      :title="
        current <= 5
          ? '算力优惠包：' + datasOne.prod_name
          : '通用算力包：' + datasTwo.prod_name
      "
    ...
    >
      <div class="codeVisible">
        ...
        <div id="qrcodeOne" v-if="this.current <= 5" ref="qrcodeOne"></div>
        <div id="qrcodeTwo" v-else ref="qrcodeTwo"></div>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    switchMemberType(type) {
      ...
      if (type == 3) {
        totalPrice = Number(
          (this.memberList[0].prod_price * this.memberList[0].shopNumber).toFixed(2)
        );
        this.datas = { ...this.memberList[0], totalPrice };
        this.open();
      } else {
        totalPrice = Number(
          (this.publicList[0].prod_price * this.publicList[0].shopNumber).toFixed(2)
        );
    ...
    open() {
      ...
      if (this.datas.resetPay == 1) {
        ...
        if (this.datas.payment_method == "1") {
          ...
          clearInterval(this.timer);
          this.order_id = this.datas.order_id;
          ...
          setTimeout(() => {
            ...
            clearInterval(this.timer);
            this.order_id = this.datasOne.order_id;
            this.timer = setInterval(() => {
            ...
            try {
              ...
              if (this.current < 6) {
                let totalPriceOne = Number(
                  (this.memberList[0].prod_price * this.memberList[0].shopNumber).toFixed(
                    2
                  )
                );
                ...
                totalPrice = Number(
                  (this.publicList[0].prod_price * this.publicList[0].shopNumber).toFixed(
                    2
                  )
                );
...
.topPay {
  ...
  box-shadow: 0px 0px 10px 0px rgba(98, 98, 98, 0.05);
  > div:nth-child(1) {
    font-weight: 500;
    font-size: 18px;
    color: #333333;
