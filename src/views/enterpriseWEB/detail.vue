  ...
  <div class="enterprise-detail">
    ...
    <div>
        ...
        <div class="steps">
          <div style="display: flex;align-items: center;" v-for="(item, i) in setpsList" :key="i">
            ...
            <div v-else class="round-dot" style="background-color: #F2F3F5;color: #4E5969;" @click="selectActicve(i)">{{ i
              + 1 }}</div>
            <div class="contetns" @click="selectActicve(i)">{{ item }}</div>
      ...
      <div class="basic_information" v-if="active == 0">
        <company-basic @nextstep="nextStep" :data="companyBasicMsg.basicData" :companyCode="company_code" @enterprisecode="enterpriseCode"></company-basic>
      </div>
...
import { business, enterpriseRetrieve, companyProductMsg, companyNewsMsg, companyKnowledgeMsg, companyBasicMsg } from '../../api/chatMG'
...
export default {
  ...
  methods: {
    ...
    enterpriseCode(code) {
      ...
      this.randomProductNumber = 100
      this.randomNewsNumber = 100
      this.randomBaseNumber = 100
    },
    selectActicve(type) {
      this.active = type
    },
    ...
    completeWrite(data) {
      ...
      if (this.productNumber == this.companyBasicMsg.productData.length) {
        ...
        //   website_list: website,
        // }
        if (this.randomProductNumber != 10 && this.company_code != '') {
    ...
    comleteNewsWrite(data) {
      ...
      if (this.newsNumber == this.companyBasicMsg.newsData.length) {
        ...
        //   website_list: website,
        // }
        if (this.randomNewsNumber != 10 && this.company_code != '') {
      ...
      if (this.knowledgeBases == this.companyBasicMsg.baseData.length) {
        ...
        if (this.randomBaseNumber != 10) {
          if (this.company_code == '') {
            let miniCodeList = []
            this.companyBasicMsg.basicData.miniCodeList.forEach(v => {
              miniCodeList.push({
                file_name: v.name,
                file_url: v.file_url
              })
            ...
            this.companyBasicMsg.basicData.officialAccountList.forEach(v => {
            ...
            let companyLogoList = []
            this.companyBasicMsg.basicData.companyLogoList.forEach(v => {
            ...
            this.companyBasicMsg.basicData.companyPictureList.forEach(v => {
            ...
            let fileList = []
            this.companyBasicMsg.basicData.fileList.forEach(v => {
              fileList.push({
                file_name: v.name,
                file_url: v.file_url
              })
            ...
            let pictureList = []
            this.companyBasicMsg.basicData.pictureList.forEach(v => {
              pictureList.push({
                file_name: v.name,
                file_url: v.file_url
              })
            ...
            let videoList = []
            this.companyBasicMsg.basicData.videoList.forEach(v => {
              videoList.push({
                file_name: v.name,
                file_url: v.file_url
              })
            ...
            let networkAddress = []
            this.companyBasicMsg.basicData.link.forEach(v => {
              networkAddress.push({
                file_name: v,
                file_url: v
              })
            ...
            let data = {
              "action": "submit",
              "company_name": this.companyBasicMsg.basicData.companyName, // 公司名称
              "company_abbreviation": this.companyBasicMsg.basicData.companyAbbreviation, // 公司简称
              "position": this.companyBasicMsg.basicData.position, // 职位
              "industry_code": this.companyBasicMsg.basicData.industry, // 行业编号
              "registered_address": this.companyBasicMsg.basicData.registerAddress, // 注册地址
              "company_desc": this.companyBasicMsg.basicData.companyDescribe, // 公司描述
              "company_url": this.companyBasicMsg.basicData.companyWebAddress, // 公司网址
              "ipc_code": this.companyBasicMsg.basicData.ICPNumber, // ipc备案号
              "company_mailbox": this.companyBasicMsg.basicData.email, // 公司邮箱
              "company_address": this.companyBasicMsg.basicData.companyAddress, // 公司地址
              "company_mobile": this.companyBasicMsg.basicData.phone, // 公司电话
            ...
            if (this.companyBasicMsg.basicData.companyName != '' && this.companyBasicMsg.basicData.companyAbbreviation != '') {
              companyBasicMsg(data).then(res => {
                this.$store.dispatch('app/changeSubmitCompany', '2')
                if (res.code == 20000) {
                  this.company_code = res.data
                  localStorage.setItem('ENTERPRRISE_CODE_SWITCH', res.data)
                  localStorage.setItem('ENTERPRRISE_NAME_SWITCH', this.companyBasicMsg.basicData.companyName)
                  localStorage.setItem('ENTERPRRISE_DESC_SWITCH', this.companyBasicMsg.basicData.companyDescribe)
                  localStorage.setItem('ENTERPRRISE_ICON_SWITCH', '')
                  this.$store.dispatch('user/changeEnterpriseId', res.data)
                  this.submitKnowledge(knowledge_list)
                } else {
                  this.$message.error(res.msg)
                }
              })
