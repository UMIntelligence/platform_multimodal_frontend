  ...
  <div :style="backImg" class="poem">
    <div class="content">
      <div class="cont_line">
        ...
        <div class="more_img" v-if="srcList.length > 0">
          <el-image
            class="other_img"
            :src="srcList[1]"
            :preview-src-list="srcList"
    ...
    >
      <div class="diolog_prot" v-if="promptList.length > 0">
        <div class="prot_text" v-for="(item, index) in promptList" :key="index">
          ...
          <span
            class="prot_l"
            :title="item.prompt_content"
            @click="selectPrompt(item.prompt_content)"
            >{{ item.prompt_content }}</span
          >
    ...
    >
      <div class="diolog_cont">
        <div class="cont_title">输入抖音直播间ID</div>
        <el-input v-model="liveId" placeholder="请输入id"></el-input>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="liveVisible = false">取 消</el-button>
        <el-button type="primary" @click="startLive">开 启</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      dialogVisible: false,
      userSlove:
        "你现在是诗仙李白，擅长写诗，现在给姓名为“欧阳若云”的好友，写一首七言绝句的藏头诗,共四句，每句7个字，要求积极向上，正向，有趣，好玩，可以适当使用夸张的手法",
      userName: "欧阳若云",
  ...
  created() {
    ...
    let img_url = localStorage.getItem("backImg3");
  ...
  mounted() {
    ...
    const headImg1 = httpUrls.ossUrl + "name_shanshui/欧阳0.png";
    this.srcList.push(headImg1);
    const headImg2 = httpUrls.ossUrl + "name_building/欧阳0.jpg";
    this.srcList.push(headImg2);
    const headImg3 = httpUrls.ossUrl + "name_lanhua/欧阳0.png";
    this.srcList.push(headImg3);
  },
  ...
  methods: {
    ...
    startLive() {
      ...
      this.socket = new WebSocket(
        `ws://192.168.1.109:8091/ws/dy_async/poem?dy_id=${this.liveId}`
      );
      // 监听socket连接
      this.socket.onopen = function () {
        ...
        that.socket.send(JSON.stringify(val));
        that.sendVal();
        console.log("建立连接成功");
      };
      // 监听socket错误信息
    ...
    socketMessage(res) {
      ...
      switch (data.type) {
        case "L":
          ...
          break;
        case "D":
          console.log(data.msg, "直播间生成的姓名");
          this.sendValue = data.msg;
          this.sendMsg();
        case "S":
    ...
    async vipSend(bool = "") {
      ...
      val = {
        ...
        msg_list: JSON.stringify([
          { role: "user", content: send_text, images: [] },
        ]),
      };
    ...
    checkName(uname) {
      ...
      const surname_two =
        "万俟|司马|上官|欧阳|夏侯|诸葛|闻人|东方|赫连|皇甫|尉迟|公羊|澹台|公冶|宗政|濮阳|淳于|单于|太叔|申屠|公孙|仲孙|轩辕|令狐|锺离|宇文|长孙|慕容|鲜于|闾丘|司徒|司空|丌官|司寇|子车|微生|颛孙|端木|巫马|公西|漆雕|乐正|壤驷|公良|拓拔|夹谷|宰父|谷梁|段干|百里|东郭|南门|呼延|羊舌|梁丘|左丘|东门|西门|南宫";
      ...
      if (
        patt1.test(surname_one) == false &&
        patt2.test(surname_two) == false
      ) {
        this.firstName = uname.slice(0, 1);
      }
    ...
    changeImg() {
      localStorage.setItem("backImg3", this.imageUrl);
